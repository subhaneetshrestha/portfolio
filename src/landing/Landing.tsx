...
export default function Landing() {
  ...
  useEffect(() => {
    ...
    // buildScene() now loads real model files (src/three/models.ts) — a real,
    // if small (~280KB), async wait, unlike the old fully-synchronous version.
    // `cancelled` guards against wiring up a renderer for a component that
    // unmounted before the load finished (fast route changes, tests).
    let cancelled = false;
    let teardown: (() => void) | undefined;

    buildScene(readPalette()).then(({ scene, camera, homeCameraPosition }) => {
      if (cancelled) {
        disposeScene(scene); // never rendered, but still owns loaded geometry
        return;
      }
